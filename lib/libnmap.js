  ...
  var tools = tools || {
    ...
    adapters: function(obj) {
      ...
      for (var iface in adapters) {
        ...
        adapter = adapters[iface][adapters[iface].length-1];
      console.log('ADDRESS >>>: ', adapter.address);
        ...
        if (!adapter.internal) {
          ...
          if (adapter.netmask) {
            ...
            /* Convert netmask to CIDR notation if IPv4 */
            if (validation.test(validation.net.IPv4, netmask)) {
              netmask = '24', new v4.Address(netmask).subnet.substring(1)
              adapter.address = adapter.address.replace(/[0-9]{1,3}$/, '0');
              console.log('IPv4 netmask: ', netmask);
            }
          ...
          console.log(adapter.address+'/'+netmask);
